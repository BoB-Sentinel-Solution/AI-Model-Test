# llama32_bllossom_tiny10.py
# -----------------------------------------------------------
# pip install -U transformers accelerate torch sentencepiece
# (저사양이면 실행 시 --int4 권장)
# 예) python llama32_bllossom_tiny10.py --int4 --max_new_tokens 128
# -----------------------------------------------------------
...
SYS_PROMPT = (
    ...
    "Given the user's text, return ONLY valid JSON with keys: "
    "`has_sensitive` (true/false) and `entities` (list of objects with `type` and `value`). "
    "Copy values verbatim from the user text. No extra text."
)
...
def build_chat(tokenizer, user_text: str) -> str:
    ...
    # Llama 3.2 계열은 chat_template가 포함되어 있음
...
def infer_once(tokenizer, model, prompt: str, max_new_tokens=96) -> dict:
    ...
    gen_kwargs = dict(
        **inputs,
        ...
        do_sample=False,  # greedy; temperature/top_p 전달 안함(경고 제거)
...
def main():
    ...
    ap.add_argument("--model", default="Bllossom/llama-3.2-Korean-Bllossom-3B")  # 기본 3B Bllossom
