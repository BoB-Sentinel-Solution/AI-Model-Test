# mistral7b_test.py  — mistralai/Mistral-7B-Instruct-v0.3
# -----------------------------------------------------------
...
# 실행 예:
#   python mistral7b_test.py                 # 전체 프롬프트 전부
#   python mistral7b_test.py --int4          # 4bit 양자화
#   python mistral7b_test.py --from_file "모델 선정을 위한 테스트.txt"
#   python mistral7b_test.py --limit 15
# -----------------------------------------------------------
...
def load_model(model_id: str, int4: bool):
    ...
    def compat_load(qconfig=None, prefer_fp16=False):
        ...
        try:
            ...
            # 구버전 Transformers 호환: attn_implementation 인자 제거 후 재시도
...
def infer_once(tok, model, prompt: str, max_new_tokens=96):
    ...
    return {"raw": out, "json": parse_json(out), "ttft_ms": ttft_ms, "tok_s": rate}
...
def main():
    ...
    for i, p in enumerate(prompts, 1):
        ...
        print(
            "ttft_ms:",
            f"{r['ttft_ms']:.2f}" if r["ttft_ms"] else "NA",
            "| tok/s:",
            f"{r['tok_s']:.2f}" if r["tok_s"] else "NA",
        )
        print("output:", r["raw"])
