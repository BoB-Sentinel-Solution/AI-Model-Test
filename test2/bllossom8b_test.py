# bllossom8b_test.py  — MLP-KTLim/llama-3-Korean-Bllossom-8B
# -----------------------------------------------------------
...
# 실행 예:
#   python bllossom8b_test.py                 # 전체 프롬프트 전부
#   python bllossom8b_test.py --int4          # 4bit 양자화
#   python bllossom8b_test.py --from_file "모델 선정을 위한 테스트.txt"
#   python bllossom8b_test.py --limit 15
# -----------------------------------------------------------
...
def load_model(model_id, int4):
    ...
    def compat_load(qconfig=None, prefer_fp16=False):
        ...
        try:
            ...
            # 구버전 Transformers: attn_implementation 제거 후 재시도 → 그래도 안 되면 torch_dtype 사용
...
def infer_once(tok, model, prompt, max_new_tokens=96):
    ...
    return {"raw": out, "json": parse_json(out), "ttft_ms": ttft_ms, "tok_s": tok_s}
